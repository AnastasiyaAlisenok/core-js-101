...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const funcArguments = args.map((item) => JSON.stringify(item));
    const startCall = `${nameFunction}(${funcArguments}) starts`;
    logFunc(startCall);
    const result = func(...args);
    const endCall = `${nameFunction}(${funcArguments}) ends`;
    logFunc(endCall);
    return result;
  };
}
